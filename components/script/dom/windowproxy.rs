...
use js::glue::{GetProxyPrivate, GetProxyReservedSlot, SetProxyReservedSlot, SetDataPropertyDescriptor};
...
unsafe extern "C" fn getOwnPropertyDescriptor(
    ...
    if let Some((window, attrs)) = window {
        ...
        SetDataPropertyDescriptor(desc, val.handle().into(), attrs);
        *is_none = false;
        //desc.value = val.get();
    ...
    /*if !JS_GetOwnPropertyDescriptorById(cx, target.handle().into(), id, desc, is_none) {
        return false;
    }*/
